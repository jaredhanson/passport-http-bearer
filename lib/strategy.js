 ...
 * @param {boolean} [options.allowQueryParameter=true] - When `true`, the strategy
 *          also attempts to obtain `access_token` from a query string. Otherwise
 *          the usage of `access_token` query parameter is forbidden and strategy will fail.
...
function Strategy(options, verify) {
  ...
  this._allowQueryParameter =
    typeof options.allowQueryParameter === 'undefined' ? true : options.allowQueryParameter;
}
...
Strategy.prototype.authenticate = function(req) {
  ...
  if (req.query && req.query.access_token) {
    if (token || !this._allowQueryParameter) {
      return this.fail(400);
    }
